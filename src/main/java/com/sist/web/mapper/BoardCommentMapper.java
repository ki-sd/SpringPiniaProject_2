...
public interface BoardCommentMapper {
	...
	@Select("""
			SELECT id,group_id,group_step,group_tab
			FROM bootcomment
			WHERE no=#{no}
			""")
	...
	@Insert("""
			INSERT INTO bootcomment(no,board_no,id,name,msg,regdate,group_id,group_step,group_tab,root,depth)
			VALUES(#{no},#{board_no},#{id},#{name},#{msg},SYSDATE,#{group_id},#{group_step},#{group_tab},#{root},0)
			""")
